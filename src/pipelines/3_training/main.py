...
# Limitar hilos para evitar alertas del backend loky en Windows
...
class TrainConfig:
    """Configuración inmutable del periodo de entrenamiento."""
...
class DataLoader:
    ...
    def load(self) -> pd.DataFrame:
        if not self.data_path.exists():
            raise FileNotFoundError(f"Dataset de entrada no encontrado en: {self.data_path.resolve()}")
...
class ArtifactLoader:
    """Carga contratos y metadatos del pipeline."""
...
class ExpandingWindowCVSplitter:
    """Genera pliegues cronológicos dentro del conjunto de entrenamiento."""
...
class DemandForecastModel:
    """Regressor LightGBM en espacio de diferencias (YoY delta)."""
...
class HyperparameterTuner:
    ...
    def _objective(self, trial: optuna.Trial, folds: list[tuple[pd.DataFrame, pd.DataFrame]]) -> float:
        ...
        for fold_train, fold_val in folds:
            ...
            level_pred = fold_val["lag_1"].to_numpy() + diff_pred
            fold_wapes.append(
    ...
    def tune(self, train_df: pd.DataFrame) -> dict[str, Any]:
        ...
        if not folds:
            logger.warning("Historial insuficiente para CV; utilizando parámetros por defecto.")
        ...
        logger.info("Optimización de hiperparámetros completada | Mejor CV WAPE: %.4f", study.best_value)
        logger.info("Parámetros seleccionados: %s", study.best_params)
        return study.best_params


class ArtifactManager:
    """Persistencia del modelo entrenado y sus hiperparámetros."""
    ...
    def save(
        ...
        logger.info("Modelo entrenado guardado exitosamente en: %s", output_model_path.resolve())

        with open(artifacts_dir / "best_params.json", "w", encoding="utf-8") as f:
            json.dump(best_params, f, indent=4)

        logger.info("Hiperparámetros exportados en: %s", artifacts_dir.resolve())


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Paso 3 MLOps: Entrenamiento y optimización de hiperparámetros."
    )
    ...
    parser.add_argument("--output_model_path", type=Path, required=True, help="Ruta de salida para el modelo (.joblib).")
    parser.add_argument("--artifacts_dir", type=Path, required=True, help="Directorio de artefactos de entrenamiento.")
    parser.add_argument("--n_trials", type=int, default=40, help="Número de iteraciones para Optuna.")
...
def main() -> None:
    ...
    # Filtrar estrictamente la ventana de entrenamiento (<= 2015)
    train_df = clean_df.loc[clean_df[train_config.year_col] <= train_config.train_end_year].copy()
    logger.info("Registros de entrenamiento filtrados (<= %s): %s filas", train_config.train_end_year, len(train_df))
    ...
    # Ajustar modelo final con todo el set de entrenamiento
    model = DemandForecastModel(
        feature_columns=feature_columns,
        categorical_features=categorical_features,
        diff_target_col=diff_target_col,
    ...
    # Guardar únicamente los artefactos de entrenamiento
